...
namespace Codewars.Solutions._6_Kyu
{
    ...
    class PiApproximation
    {
        public static ArrayList iterPi(double epsilon)
        {
            int iter = 0;
            double piDiv4 = 0;
            for (int denom = 1, sign = 1; Math.Abs(piDiv4 * 4 - Math.PI) > epsilon; iter++, denom++, sign *= -1)
            {
                piDiv4 += sign * 1 / (double)(iter + denom);
            }
            return new ArrayList() { iter, Math.Round(piDiv4 * 4, 10) };
        }
    }
}
